...
all parsed structures are found in (slice and map) members therein, accessible with methods like AllSubClassOfs(), or AllClassDecls(), or ClassDecl("example.com/Pizza"). These methods are in the "Axioms" and "Decls" interfaces of the store subpackage.
...
The API may change and Gopfp does not use a semantic version yet.
